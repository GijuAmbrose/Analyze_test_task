class ArchivalNdsom < ApplicationRecord
  ...
  scope  :fetch_analyse_data, -> {select("securities.isin", "interest_frequencies.frequency", "securities.interest_frequency_id", "securities.issue_date", "securities.maturity_date", "securities.id", "archival_ndsoms.ytm", "archival_ndsoms.id as a_id")
                                  .joins("INNER JOIN securities ON securities.isin = archival_ndsoms.isin
                                  JOIN interest_frequencies ON interest_frequencies.id = securities.interest_frequency_id")
                                  .uniq(&:isin).take(15)}
